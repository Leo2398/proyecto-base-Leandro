...
class ProductFamilyService implements IProductFamilyService {
  ...
  Future<List<ProductFamilyModel>> getAll() async {
    try {
      ...
      if (result.rows.isNotEmpty) {
        developer.log(
          'productfamily - Primera fila: ${result.rows.first.assoc()}',
        );
      }
      ...
      final families = result.rows
        ...
        try {
          ...
          developer.log(
            'Error parseando productfamily: $e, data: ${row.assoc()}',
          );
          rethrow;
        }
      })
  ...
  @override
  Future<bool> saveProducerFamilies(int producerID, List<int> familyIDs) async {
    try {
      final conn = await _db.getConnection();
